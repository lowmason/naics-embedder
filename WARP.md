...
`ruff check src tests` and `./scripts/format_code.sh --check --all` both pass; keep it that way.
Format the files your change touches and don't mass-fix unrelated code (never run
`./scripts/format_code.sh --all` in a feature PR). Never run `ruff format`: it switches every string
to double quotes and puts 2 blank lines between top-level definitions, so it rewrites code you
didn't touch. See CLAUDE.md for the full style guide.
