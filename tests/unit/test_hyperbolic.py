...
class TestLorentzDistance:
    ...
    def test_batched_distance_correctness(self, sample_lorentz_embeddings):
        ...
        for i in range(batch_size):
            for j in range(k):
                pairwise_dist[i, j] = distance_fn(
                    anchor[i:i + 1], points[i, j:j + 1, :]
                ).item()
...
class TestManifoldValidity:
    ...
    @pytest.mark.parametrize('curvature', [0.1, 0.5, 1.0, 5.0, 10.0])
    def test_validity_check_with_different_curvatures(
        self, sample_tangent_vectors, curvature
    ):
...
class TestHyperbolicProperties:
    ...
    def test_exp_produces_valid_manifold_property(self, curvature):
        ...
        is_valid, _, _ = check_lorentz_manifold_validity(
            hyp, curvature=curvature, tolerance=1e-2
        )
