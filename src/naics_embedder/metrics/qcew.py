...
def run_qcew_multilevel_benchmark(
    ...
    if successful_levels:
        successful_results = [
            results[f'level_{level}_{NAICS_LEVEL_NAMES.get(level, "unknown")}']
            for level in successful_levels
        ]
        embed_r2_values = [level_results['embedding']['r2'] for level_results in successful_results]
        onehot_r2_values = [level_results['one_hot']['r2'] for level_results in successful_results]
        hybrid_r2_values = [level_results['hybrid']['r2'] for level_results in successful_results]

        embed_rmse_values = [level_results['embedding']['rmse'] for level_results in successful_results]
        onehot_rmse_values = [level_results['one_hot']['rmse'] for level_results in successful_results]

        results['summary'] = {
            'levels_evaluated': {
                str(level): NAICS_LEVEL_NAMES.get(level, 'unknown') for level in successful_levels
            },
            'embedding': {
