...
def _read_xlsx_bytes(data: bytes, sheet: str, schema: Dict[str, pl.DataType],
                     cols: Dict[str, str]) -> pl.DataFrame:
    """
    ...
    return pl.read_excel(BytesIO(data),
                         ...
                         schema_overrides=schema).rename(mapping=cols)
...
def _read_xlsx(
    ...
    data = _download_with_retry(url, max_retries, initial_delay,
...
def _download_files(
    ...
    schema_descriptions = {
        k: getattr(pl, v)
        for k, v in cfg.schema_descriptions.items()
    }
    schema_exclusions = {
        k: getattr(pl, v)
        for k, v in cfg.schema_exclusions.items()
    }

    # NAICS titles
    titles_df = _read_xlsx(url=cfg.url_codes,
    ...
    examples_df = _read_xlsx(url=cfg.url_index,
        ...
        for col, df in df_list:
            dfs.append(
                df.with_columns(  # type: ignore
                    code=pl.when(pl.col("code").eq("31-33")).then(
                        pl.lit("31")).when(pl.col("code").eq("44-45")).then(
                            pl.lit("44")).when(
                                pl.col("code").eq("48-49")).then(
                                    pl.lit("48")).otherwise(pl.col("code"))))

            logger.info(
                f"  {col} observations: {df.height: ,}")  # type: ignore
...
def _get_descriptions_1(
        descriptions_df: pl.DataFrame) -> Tuple[pl.DataFrame, pl.DataFrame]:
    ...
    descriptions_2 = (descriptions_1.with_columns(
        description=pl.col("description").str.split("\r\n").list.eval(
            pl.element().filter(pl.element().str.len_chars() > 0))
    ).explode("description").with_columns(
        description_id=pl.col("description").cum_count().over("code")).select(
            "code", "description_id", "description").filter(
    ...
    descriptions_3 = descriptions_2.select(
        ...
        description=pl.col("description").str.strip_prefix(
            " ").str.strip_suffix(" ").str.replace_all(
                "NULL", "", literal=True).str.replace_all(
                    r"See industry description for \d{6}\.",
                    "").str.replace_all(r"<.*?>", "").str.replace_all(
                        r"\xa0", " ").str.replace_all(
                            ".", ". ", literal=True).str.replace_all(
                                "U. S. ", "U.S.",
                                literal=True).str.replace_all(
                                    "e. g. ,", "e.g.,",
                                    literal=True).str.replace_all(
                                        ...
                                        literal=True).str.replace_all(
                                            ...
                                            literal=True).str.replace_all(
                                                ...
                                                literal=True).str.replace_all(
                                                    ...
                                                    "44",
                                                    literal=True).str.
        replace_all("48-49", "48", literal=True).str.replace_all(
            r"\s{2,}", " ").str.strip_prefix(" ").str.strip_suffix(" "),
    )

    logger.info("Descriptions:")
    logger.info(f"  Number: {descriptions_1.height: ,}")
    logger.info(
        f"  Number (split on paragraphs): {descriptions_3.height: ,}\n")
...
def _get_exclusions(exclusions_df: pl.DataFrame, descriptions_3: pl.DataFrame,
    ...
    exclusions_1 = exclusions_df.filter(
        pl.col("excluded").str.contains(r" \d{2,6}"), )

    # Aggregate exclusions by code
    exclusions_2 = (exclusions_1.group_by(
        "code", maintain_order=True).agg(excluded=pl.col("excluded")).select(
            code=pl.col("code"),
    ...
    exclusions_3 = descriptions_3.filter(
        pl.col("description_id").max().over("code").eq(
            pl.col("description_id")),
        pl.col("description").str.contains_any(
    ...
    exclusions_4 = (pl.concat([exclusions_2, exclusions_3]).filter(
        pl.col("description").is_not_null()).with_columns(
            digit=pl.col("description").str.extract_all(r" \d{2,6}").list.eval(
                pl.element().str.strip_prefix(" ")).list.set_intersection(
                    codes).list.drop_nulls()).filter(
                        pl.col("digit").list.len().gt(0)))

    # Final exclusions DataFrame
    exclusions = (exclusions_4.explode("digit").select(
        level=pl.col("code").str.len_chars().cast(pl.UInt8),
        code=pl.col("code"),
        excluded=pl.col("description"),
        excluded_codes=pl.col("digit"),
    ).sort("level",
           "code").group_by("level", "code", maintain_order=True).agg(
               excluded=pl.col("excluded"),
               excluded_codes=pl.col("excluded_codes")).with_columns(
                   excluded=pl.col("excluded").list.join(" ")))

    exclusions_cnt = (exclusions.with_columns(excluded_count=pl.col(
        "excluded_codes").list.len()).get_column("excluded_count").sum())
...
def _get_examples(
    ...
    # Example spreadsheet
    examples_1 = (examples_df.filter(
        pl.col("code").is_in(codes)).sort("code").group_by(
            "code", maintain_order=True).agg(examples_1=pl.col("examples")))

    # Identify where 'Illustrative Examples:' section begins
    examples_2 = descriptions_2.filter(
        pl.col("description").str.contains("Illustrative Examples:")).select(
            code=pl.col("code"), example_id=pl.col("description_id"))

    # Extract examples that appear after 'Illustrative Examples:' marker
    examples_3 = (descriptions_3.join(
        examples_2, how="inner", on="code").filter(
            pl.col("example_id").lt(pl.col("description_id"))).group_by(
                "code", maintain_order=True).agg(
    ...
    examples_4 = examples_1.join(examples_3,
                                 how="full",
                                 on="code",
                                 coalesce=True).select(code=pl.col("code"),
                                                       examples=pl.coalesce(
                                                           "examples_1",
                                                           "examples_2"))

    examples = examples_4.select(code=pl.col("code"),
                                 examples=pl.col("examples").list.join("; "))

    examples_cnt = (examples_4.with_columns(example_cnt=pl.col(
        "examples").list.len()).get_column("example_cnt").sum())
...
def _get_descriptions_2(
    ...
    descriptions_4 = (descriptions_3.join(
        descriptions_exclusions, how="anti",
        on=["code", "description_id"
            ]).join(descriptions_examples, how="left", on="code").with_columns(
                pl.col("description_id_min").fill_null(999)).filter(
                    pl.col("description_id").lt(pl.col("description_id_min"))
                ).group_by("code", maintain_order=True).agg(
                    pl.col("description")).with_columns(
                        description=pl.col("description").list.join(" ")))

    # Separate complete descriptions from missing ones
    description_complete_1 = descriptions_4.filter(
    ...
    description_5_missing = descriptions_4.filter(
        ...
        pl.col("description").eq("")).select(
            code=pl.col("code").str.pad_end(6, "0"))
    ...
    description_5_complete = (description_5_missing.join(
        description_complete_1, how="inner",
        on="code").with_columns(code=pl.col("code").str.slice(0, 5)).select(
            code=pl.col("code"),
            description=pl.col("description").str.replace(
                "This industry", "This NAICS industry", literal=True),
        ))

    description_complete_2 = pl.concat(
        [description_complete_1, description_5_complete])
    ...
    description_4_complete = (pl.concat([
        ...
        description=pl.col("description").str.replace(
            "This industry", "This industry group",
            literal=True).str.replace("This NAICS industry",
                                      "This industry group",
                                      literal=True),
    ).unique(subset=["code"]))
    ...
    logger.info(f"  Missing (level 4): {description_4_missing.height: ,}")
    logger.info(
        f"  Filled missing (level 4): {description_4_complete.height: ,}")
    logger.info(f"  Missing (level 5): {description_5_missing.height: ,}")
    logger.info(
        f"  Filled missing (level 5): {description_5_complete.height: ,}")
...
def download_preprocess_data() -> pl.DataFrame:
    ...
    titles_df, descriptions_df, examples_df, exclusions_df = _download_files(
        cfg)
    ...
    examples, descriptions_examples = _get_examples(examples_df, codes,
                                                    descriptions_2,
                                                    descriptions_3)

    descriptions = _get_descriptions_2(descriptions_3, descriptions_exclusions,
                                       descriptions_examples)

    # Join all components and write final output
    naics_final = (titles.join(descriptions, how="inner", on="code").join(
        exclusions, how="left",
        on="code").join(examples, how="left", on="code").select(
            index=pl.col("index"),
            level=pl.col("level"),
            code=pl.col("code"),
            title=pl.col("title"),
