...
class DirConfig(BaseModel):
    ...
    conf_dir: str = Field(default='./conf', description='Directory for config files')
    data_dir: str = Field(default='./data', description='Directory containing data files')
    docs_dir: str = Field(default='./docs', description='Directory containing data files')
...
class DownloadConfig(BaseModel):
    ...
    sheet_codes: str = Field(
        default='tbl_2022_title_description_coun', description='Sheet name for codes'
    )
    sheet_index: str = Field(default='2022NAICS', description='Sheet name for index')
    sheet_descriptions: str = Field(
        default='2022_NAICS_Descriptions', description='Sheet name for descriptions'
    )
    sheet_exclusions: str = Field(
        default='2022_NAICS_Cross_References', description='Sheet name for exclusions'
    )
...
class RelationsConfig(BaseModel):
    ...
    output_parquet: str = Field(
        default='./data/naics_relations.parquet', description='Output relations parquet file'
    )
...
class DistancesConfig(BaseModel):
    ...
    distances_parquet: str = Field(
        default='./data/naics_distances.parquet', description='Output distances parquet file'
    )
...
class TripletsConfig(BaseModel):
    ...
    distances_parquet: str = Field(
        default='./data/naics_distances.parquet', description='Input distances parquet file'
    )
    relations_parquet: str = Field(
        default='./data/naics_relations.parquet', description='Input relations parquet file'
    )
    output_parquet: str = Field(
...
class DataConfig(BaseModel):
    ...
    relations: RelationsConfig = Field(
        default_factory=RelationsConfig, description='Relations configuration'
    )
    distances: DistancesConfig = Field(
        default_factory=DistancesConfig, description='Distances configuration'
    )
...
class StreamingConfig(BaseModel):
    ...
    triplets_parquet: str = Field(
        default='./data/naics_training_pairs', description='Path to training pairs directory'
    )
    tokenizer_name: str = Field(
        default='sentence-transformers/all-MiniLM-L6-v2', description='HuggingFace tokenizer name'
    )
...
class MoEConfig(BaseModel):
    ...
    hidden_dim: int = Field(default=1024, gt=0, description='Hidden dimension for expert networks')
    load_balancing_coef: float = Field(
        default=0.01, ge=0, le=1, description='Load balancing loss coefficient'
    )
...
class GraphConfig(BaseModel):
    ...
    output_dir: str = Field(
        default='./output/hgcn/', description='Output directory for checkpoints and logs'
    )
    output_parquet: str = Field(
    ...
    dropout: float = Field(default=0.10, ge=0, le=1, description='Dropout rate')
    ...
    # Training parameters
    num_epochs: int = Field(default=8, gt=0, description='Number of training epochs')
    epoch_every: int = Field(default=1, gt=0, description='Log every N epochs')
    batch_size: int = Field(default=64, gt=0, description='Batch size')
    lr: float = Field(default=0.00075, gt=0, description='Learning rate')
    warmup_ratio: float = Field(
        default=0.2, ge=0, le=1, description='Fraction of epochs for warmup'
    )
    temperature_start: float = Field(
        default=0.10, gt=0, description='Starting temperature for loss scaling'
    )
    temperature_end: float = Field(
        default=0.08, gt=0, description='Ending temperature for loss scaling'
    )
    ...
    # Loss parameters
    triplet_margin: float = Field(default=1.0, gt=0, description='Triplet loss margin')
    ...
    pct_excluded: float = Field(
        default=0.0, ge=0, le=1, description='Percentage of excluded negatives'
    )
    pct_hard: float = Field(default=0.05, ge=0, le=1, description='Percentage of hard negatives')
    pct_medium: float = Field(
        default=0.15, ge=0, le=1, description='Percentage of medium negatives'
    )
    pct_easy: float = Field(default=0.55, ge=0, le=1, description='Percentage of easy negatives')
    pct_unrelated: float = Field(
        default=0.25, ge=0, le=1, description='Percentage of unrelated negatives'
    )

    n_positive_samples: int = Field(default=2048, gt=0, description='Number of positive samples')
    allowed_relations: Optional[List[str]] = Field(
        default=None, description='Allowed relations for filtering'
    )
    min_code_level: Optional[int] = Field(
        default=None, ge=2, le=6, description='Minimum code level'
    )
    max_code_level: Optional[int] = Field(
        default=None, ge=2, le=6, description='Maximum code level'
    )
    ...
    pin_memory: bool = Field(default=True, description='Pin memory for data loader')

    device: str = Field(default='auto', description='Device string')
