...
def sector_tree(sector: str, input_parquet: str) -> nx.DiGraph:

    code_6 = sector_codes(sector, input_parquet)
...
def compute_tree_metadata(tree: nx.DiGraph, root: str) -> Tuple[Dict, Dict, Dict]:
...
def find_common_ancestor(i: str, j: str, ancestors: Dict[str, List[str]]) -> Optional[str]:
...
def get_distance(
    ...
    common_ancestor = find_common_ancestor(i, j, ancestors)
...
def distance_stats(distances_df: pl.DataFrame):       
    ...
    def _render_triplet_table(rows):
        ...
        title = Text('Distance Statistics', style='bold')
        table = Table(
            title=title,
            title_justify='left',
            show_lines=True
...
def parquet_stats(parquet_df: pl.DataFrame, output_parquet: str) -> None:
    ...
    logger.info(f'Parquet observations: {parquet_df.height: ,}\n')
    ...
    logger.info(f'{parquet_df.height: ,} NAICS pairwise distances written to:')
...
def calculate_pairwise_distances() -> None:
    ...
    df_list = []
    for sector in sectors(cfg.input_parquet):

        G = sector_tree(sector, cfg.input_parquet)

        pairs = [(i, j) if int(i) < int(j) else (j, i) for i, j in combinations(G.nodes, 2)]

        depths, ancestors, parents = compute_tree_metadata(G, sector)

        distances = []
        for i, j in sorted(pairs, key=lambda x: (x[0], x[1])):
            distance = get_distance(i, j, depths, ancestors)
    ...
    distance_stats(naics_distances)

    parquet_stats(naics_distances, cfg.output_parquet)
